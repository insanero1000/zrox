    ...
    if (message.id === apply_db.get(message.guild.id, "message_id") && message.channel.id === apply_db.get(message.guild.id, "channel_id") && reaction.emoji.name === "✅") {
      try {
        ...
        //but if he is having an running application somewhere then return error
          ...
          .setDescription("You are already having a Running Application somwhere!")
          .setTitle("ERROR")
          .setFooter(es.footertext, es.footericon)
        ).catch(e => {
          message.channel.send({content: `${user}`,embed: new Discord.MessageEmbed()
            .setColor(es.wrongcolor)
            .setFooter(es.footertext, es.footericon)
    ...
    if (message.channel.id === apply_db.get(message.guild.id, "f_channel_id") && (all_finished_reactions.includes(reaction.emoji.name))) {
      try {
        ...
        if (reaction.emoji.name === "✅") {
          ...
          if(usert) await rome_old_roles(reaction, usert, apply_db);
        ...
        if (reaction.emoji.name === "❌") {
          ...
          //try to remove all roles after that continue?
          await rome_old_roles(reaction, usert, apply_db);
          ...
          try {
            message.guild.channels.create(`Ticket-${usert.username}`.substr(0, 32), {
              ...
              .then((channel) => {
                ...
                let button_user = new MessageButton().setStyle('green').setID('ticket_user').setLabel("Managee Users").setEmoji("👤")
                let button_role = new MessageButton().setStyle('green').setID('ticket_role').setLabel("Managee Roles").setEmoji("📌") 

...
module.exports = (client) => {
  ...
  client.on("messageReactionAdd", async (reaction, user) => {
    ...
    if (reaction.message.guild.id === ticket.guildid && reaction.message.id === ticket.messageid) {
      ...
      if (client.setups.get("TICKETS", "tickets").includes(user.id)) {
        ...
        try {
          var ticketchannel = reaction.message.guild.channels.cache.get(client.setups.get(user.id, "ticketid"))
  ...
  client.on("clickButton", async (button) => {
    ...
    if (client.setups.get("TICKETS", "tickets").includes(user.id)) {
      try {
        var ticketchannel = guild.channels.cache.get(client.setups.get(user.id, "ticketid"))
        if (!ticketchannel || ticketchannel == null || !ticketchannel.id || ticketchannel.id == null) throw {
          message: "NO TICKET CHANNEL FOUND AKA NO ANTISPAM"
        }
        return button.reply.send(`<:no:833101993668771842> **You already have an Ticket!** <#${client.setups.get(user.id, "ticketid")}>`, true);
      } catch {
        client.setups.remove("TICKETS", user.id, "tickets")
      }
